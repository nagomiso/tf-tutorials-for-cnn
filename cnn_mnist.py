...
def main(unused_argv):
    ...
    mnist_classifier = tf.estimator.Estimator(
        model_fn=cnn_model_func, model_dir='./mnist_cnn_model')

    # Set up logging for predictons
    ...
    # Ealuate the model and print results
    eval_input_func = tf.estimator.inputs.numpy_input_fn(
        x={'x': eval_data},
        y=eval_labels,
        num_epochs=1,
        shuffle=False)
    eval_results = mnist_classifier.evaluate(input_fn=eval_input_func)
    _print(eval_results)
